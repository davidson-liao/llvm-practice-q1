...
| 资料 | 位置/链接 | 用途 |
|------|-----------|------|
| **多规模/多配置** | 不同 N/C/H/W、tile size | 正确性 + 性能曲线 |
| **CPU 与 GPU 双路径** | 同一图跑 CPU 与 GPU | 数值一致、性能对比 |
| **Lit/FileCheck 自动化** | test 目录 | 每阶段 IR 回归测试 |

### 2.2 深化验收与性能

- 文档化：每阶段 Pass、输入/输出方言、验证方式
- 性能：GFLOPS/带宽、与 baseline 对比
- 可选：CI 中全链路 + 数值 diff、性能回归
